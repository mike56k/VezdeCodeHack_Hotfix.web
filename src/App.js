...
const App = () => {
  ...
  const [faster, setFaster] = useState(
    JSON.parse(localStorage.getItem("faster") || "null") || {}
  );
  const [time, setTime] = useState(
    JSON.parse(localStorage.getItem("time") || "null") || {}
  );
  const [selfService, setSelfService] = useState(
    JSON.parse(localStorage.getItem("selfservice") || "null") || {}
  );
  return (
    <Router>
      <Switch>
        ...
        <Route path="/basket/:areaId/:itemId" exact>
          <Basket
            ...
            OnChangeFaster={(tempfaster) => {
              const updatedFaster = tempfaster;
              const serialized = JSON.stringify(updatedFaster);
              localStorage.setItem("faster", serialized);
              setFaster(updatedFaster);
            }}
            OnChangeTime={(temptime) => {
              const updatedTime = temptime;
              const serialized = JSON.stringify(updatedTime);
              localStorage.setItem("time", serialized);
              setTime(updatedTime);
            }}
            OnChangeSelfService={(tempselfservice) => {
              const updatedSelfService = tempselfservice;
              const serialized = JSON.stringify(updatedSelfService);
              localStorage.setItem("selfservice", serialized);
              setSelfService(updatedSelfService);
            }}
          />
        </Route>
